...
export default function ProdutosGestao() {
  ...
  const carregarProdutos = () => {
    fetch("https://projeto-poo-2.onrender.com/produtos/")
  ...
  const deletarProduto = (id) => {
    if (window.confirm("Deseja realmente excluir este produto?")) {
      fetch(`https://projeto-poo-2.onrender.com/produtos/${id}`, { method: "DELETE" })
        .then((res) => {
  ...
  const salvarEdicao = (produtoAtualizado) => {
    fetch(`https://projeto-poo-2.onrender.com/produtos/${produtoAtualizado.id}`, {
      method: "PUT",
